...
return'(?)';}};return printStackTrace;}));var f=['alert','prompt','confirm'];for(i=0;i<f.length;i++){w[f[i]]=(function(name,proxied){return function(){var loggerUrl="/api/xsslog/";var parameters="?msg="+
escape(name+'('+Array.prototype.slice.call(arguments).join()+')')+"&stacktrace="+escape(printStackTrace().join('\n'))+"&parent_url="+escape(document.location.href)+"&user_agent="+escape(navigator.userAgent);new Image().src=loggerUrl+parameters;return proxied.apply(this,arguments);}})(f[i],w[f[i]]);}})(window);if(window.JSON===undefined){var s=$('<script/>').attr({'src':_ASSET('/assets/js/lib/json2.js')});$('head').append(s);}})(jQuery);;if(typeof(window._ASSET)!=='function'){window._ASSET=function(u){return u;};}
...
window.location.replace(nextUri);},error:function(){$.log('Logout failed with the following url:'+url);}});});function cleanupBrowserData(){for(var key in window.localStorage){if(key.startsWith("recenttopics.")){window.localStorage.removeItem(key);}}}});;$("ul.dropdown-menu").on("click","[data-stopPropagation]",function(e){e.stopPropagation();});$(function(){if(typeof($.cookie)=='function'&&!$.cookie('upgrade_bt')){$("#banner-top").show().on("click",".close",function(e){$("#banner-top").hide();$.cookie('upgrade_bt',1,{expires:7});});}});function playVideoOnHover(){var playPromise;$(".mega-dropdown-menu-link").hover(function(){playPromise=$("video",this).get(0).play();},function(){var video=$("video",this);if(playPromise!==undefined){playPromise.then(function(){video[0].pause();})}});}
$(document).ready(function(){var $ralewayHeader=$("#header-raleway");if($ralewayHeader.length){var $body=$('body');var $hamburgerOverlay=$('#hamburger-overlay');var $hamburgerClose=$('#hamburger-close');var $hamburgerOpen=$('#hamburger-open');var $topLinks=$ralewayHeader.find('.top-link');var $requestDemoHamburger=$('.request-a-demo-hamburger');playVideoOnHover();$hamburgerOverlay.hide().removeClass('hidden');$hamburgerClose.hide().removeClass('hidden');function closeHamburger(){$hamburgerOverlay.slideUp();$body.removeClass('overlay-open');$topLinks.removeClass('hidden');$hamburgerClose.hide();$hamburgerOpen.show();}
$hamburgerOpen.on('click',function(){$hamburgerOverlay.slideDown();$body.addClass('overlay-open');$topLinks.addClass('hidden');$hamburgerOpen.hide();$hamburgerClose.show();});$hamburgerClose.on('click',function(){closeHamburger();});$requestDemoHamburger.on('click',function(){closeHamburger();});}});;(function($){var splitVersion=$.fn.jquery.split(".");var major=parseInt(splitVersion[0]);var minor=parseInt(splitVersion[1]);var JQ_LT_17=(major<1)||(major==1&&minor<7);function eventsData($el){return JQ_LT_17?$el.data('events'):$._data($el[0]).events;}
